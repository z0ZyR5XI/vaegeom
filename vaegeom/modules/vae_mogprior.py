...
class VAEMoGPrior(VAEModule):
    """Mixture of Gaussian (MoG) prior"""

    def __init__(
        self,
        model: nn.Module,
        config: VAEMoGPriorConfig,
        *args, **kwargs):
        super().__init__(model, config)
        self.n_components = self.config.n_components
        mu = torch.randn(self.n_components, self.model.dim_latent)
        self.p_z_mu = nn.Parameter(mu)
        self.p_z_logvar = nn.Parameter(torch.randn_like(mu))
        # Logit of mixture
    ...
    @classmethod
    def create_config(cls, *args, **kwargs) -> VAEMoGPriorConfig:
        return VAEMoGPriorConfig(**kwargs)
    ...
    def create_p_z(self, x: Tensor, q: Distribution) -> Distribution:
        ...
        scale = torch.exp(0.5 * self.p_z_logvar)
        comp = Independent(Normal(self.p_z_mu, scale), 1)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        p_mu = self.p_z_mu.clone().detach()
        p_cov = self.p_z_logvar.clone().detach().exp()
        p_cov = torch.diag_embed(p_cov)
        p_w = self.p_z_w.clone().detach()
        return (
            recon_x,
            q_mu,
            q_cov,
